...
helpers.titleize = function(str) {
  ...
  const smallWords = [
    'a', 'an', 'the', 'and', 'as', 'at', 'but', 'by', 'for', 'in', 'nor', 'of', 'on', 'or', 'so', 'to', 'up', 'yet', 'via', 'with', 'between', 'from', 'above', 'below', 'after', 'around', 'outside', 'toward', 'through', 'into', 'using'
  ];
  ...
  for (let i = 0; i < words.length; i++) {
    let word = words[i];
    if (word.length === 0) { // Handle multiple spaces between words
      continue;
    }

    let capitalizedWord = helpers.capitalize(word);

    // If the word (in lowercase) is in the smallWords list AND it's not the first word,
    // then convert the original word (before capitalization) to lowercase.
    if (i > 0 && smallWords.includes(word.toLowerCase())) {
      resultWords.push(word.toLowerCase());
    } else {
      resultWords.push(capitalizedWord);
    }
  }

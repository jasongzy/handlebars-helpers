...
helpers.titleize = function(str) {
  ...
  const smallWords = [
    'a', 'an', 'the', 'is', 'are', 'and', 'as', 'at', 'but', 'by', 'for', 'in', 'nor', 'of', 'on', 'or', 'so', 'to', 'up', 'yet', 'via', 'with', 'between', 'from', 'above', 'below', 'after', 'around', 'outside', 'toward', 'through', 'into', 'using'
  ];
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (currentSegment.length === 0) { // Handle multiple spaces between words
      continue;
    }

    let isFirstOverallSegment = (i === 0);
    // Check if the original previous segment from the `segments` array ended with a sentence terminator.
    let prevSegmentEndedWithSentenceTerminator = (i > 0 && segments[i-1].length > 0 && sentenceTerminatorRegex.test(segments[i-1]));
    let processedSegment;

    if (currentSegment.includes('-')) {
      // Handle hyphenated words: capitalize each part, then apply small word rules
      let parts = currentSegment.split('-');
      let capitalizedParts = parts.map((part, partIdx) => {
        if (part.length === 0) return ''; // Handle cases like "word--" or "-word"
        let capPart = helpers.capitalize(part);
        // Determine if this part should be forced capitalized
        // (i.e., first word of title, or follows a sentence terminator)
        let shouldForceCapitalize = (isFirstOverallSegment && partIdx === 0) || prevSegmentEndedWithSentenceTerminator;
        if (smallWords.includes(part.toLowerCase()) && !shouldForceCapitalize) {
          return part.toLowerCase();
        } else {
          return capPart;
        }
      });
      processedSegment = capitalizedParts.join('-');
    } else {
      let capSegment = helpers.capitalize(currentSegment);
      let shouldForceCapitalize = isFirstOverallSegment || prevSegmentEndedWithSentenceTerminator;
      if (smallWords.includes(currentSegment.toLowerCase()) && !shouldForceCapitalize) {
        processedSegment = currentSegment.toLowerCase();
      } else {
        processedSegment = capSegment;
      }
    }

    resultWords.push(processedSegment);
  }

...
helpers.titleize = function(str) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // Check the original previous segment from the `segments` array for the colon.
    let prevSegmentEndedInColon = (i > 0 && segments[i-1].length > 0 && segments[i-1].endsWith(':'));
    ...
    if (currentSegment.includes('-')) {
      ...
      let capitalizedParts = parts.map((part, partIdx) => {
        ...
        // The very first part of the very first segment is always capitalized.
        // Or any part of a segment that follows a colon.
        let isFirstWordOfTitleOrAfterColon = (isFirstOverallSegment && partIdx === 0) || prevSegmentEndedInColon;

        if (smallWords.includes(part.toLowerCase()) && !isFirstWordOfTitleOrAfterColon) {
      ...
      let isFirstWordOfTitleOrAfterColon = isFirstOverallSegment || prevSegmentEndedInColon;

      if (smallWords.includes(currentSegment.toLowerCase()) && !isFirstWordOfTitleOrAfterColon) {
